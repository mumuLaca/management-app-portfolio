 ...
 * @returns エラーコード
 */
export function checkDailyReportInput(row: DailyReport | undefined): string {
  // DailyReportがundefined
  if (!row) return "";
 ...
 * @returns エラーコード
 */
export function checkDailyReportBrank(
  targetMonth: string,
  dailyReport: DailyReport[]
): string {
  if (!dailyReport) return "EM00002";
